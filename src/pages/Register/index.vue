  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <button style="width: 100px; height: 38px" @click="requestCode">
          {{errors.first( "code" ) }}
        </button>
      ...
      <div class="controls">
        <input   type="checkbox" v-model="agree" name= "agree" v-validate="{agree:true}" :class="{ invalid:errors.has('agree')}" />
  ...
  methods: {
    ...
    async requestCode() {
      try {
        // 需要先判断数据
        const { phone } = this;
        phone && (await this.$store.dispatch("requestCode", phone));
    ...
    async userRegister() {
      ...
      // 如果表单校验全部成功，再向服务器发请求，进行注册，只要一个表单未验证成功就不会发请求
      if(success){
        try {
